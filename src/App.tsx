...
function Board({ onPointerDown }: { onPointerDown?: (e: ThreeEvent<PointerEvent>) => void }) {
  return (
    <mesh
...
export default function App() {
  const [figures, setFigures] = useState<FigureData[]>(initialFigures)
  ...
  const rotateSelected = (delta: number) => {
    ...
    updateFigure(selectedId, {
      rotationY: selected.rotationY + delta,
    })
  }
    ...
    <div style={{ width: '100%', height: '100%', display: 'flex', background: '#0d1b2a' }}>
      ...
      >
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          <button onClick={() => addFigure('person')} style={btnStyle}>
            + Person
          </button>
          <button onClick={() => addFigure('child')} style={btnStyle}>
            + Kind
          </button>
        </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            ...
            <label style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
              ...
              <input
                ...
                onKeyDown={(e) => {
                  if (e.key === 'Enter') updateFigure(selected.id, { label: labelInput })
                }}
                style={inputStyle}
              />
            </label>
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              <button onClick={() => rotateSelected(-0.4)} style={btnStyle}>
                ↺ Drehen
              </button>
              <button onClick={() => rotateSelected(0.4)} style={btnStyle}>
                Drehen ↻
            ...
            <button onClick={removeSelected} style={{ ...btnStyle, background: '#5c2a2a' }}>
              Entfernen
            </button>
          </div>
        ) : (
        ...
        <div style={{ marginTop: 'auto', display: 'flex', flexDirection: 'column', gap: 8 }}>
          <button onClick={clearBoard} style={{ ...btnStyle, background: '#3a2a1a' }}>
            Brett leeren
          </button>
          <div style={{ fontSize: 11, opacity: 0.5 }}>
            Ziehen = verschieben · Klick = auswählen
          </div>
        </div>
      </div>
